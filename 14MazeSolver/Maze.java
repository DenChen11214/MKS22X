...
public class Maze{
	...
	*/
	...
	start.setDistance(calcDis(start,end));
    }
    private int calcDis(Location one, Location two){
	int xDis = Math.abs(end.getX() - start.getX());
	int yDis = Math.abs(end.getY() - start.getY());
	return xDis + yDis;
    }
