...
public class Maze{
  ...
  */
  public Location[] getNeighbors(Location L){
    ...
    for(int i = 0 ; i < 4;i++){
	    if((maze[xCur + xDir[i]][yCur + yDir[i]] == ' ') ||
	       (maze[xCur + xDir[i]][yCur + yDir[i]] == 'E')){
        data[i] = new Location(xCur + xDir[i], yCur + yDir[i], L);
        if(isA){
          data[i].setTraveled(L.getTraveled() + 1);
          data[i].setDistance(data[i].getTraveled() + calcDis(data[i]));
          numLoc++;
        }
        else{
          data[i].setDistance(calcDis(data[i]));
          numLoc ++;
        }
	    }
    }
    Location[] data2 = new Location[numLoc];
    int index = 0;
    for(int i = 0; i < 4;i++){
	    if(data[i] != null){
        data2[index] = data[i];
  ...
  public Maze(String filename){
    ...
    */
    ...
    start.setDistance(calcDis(start));
    start.setTraveled(0);
  }
  private int calcDis(Location one){
    int xDis = Math.abs(end.getX() - one.getX());
    int yDis = Math.abs(end.getY() - one.getY());
    return xDis + yDis;
  }
